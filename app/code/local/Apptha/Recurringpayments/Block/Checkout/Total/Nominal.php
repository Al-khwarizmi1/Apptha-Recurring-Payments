 ...
 */
class Apptha_Recurringpayments_Block_Checkout_Total_Nominal extends Mage_Checkout_Block_Total_Nominal {
	/**
	 * Custom template
	 *
	 * @var string
	...
	public function getItemRowTotal(Mage_Sales_Model_Quote_Item_Abstract $quoteItem) {
		...
		$regularpayment = Mage::getSingleton ( 'checkout/session' )->getRegularPayment ();
		$regularpayment = Mage::helper ( 'directory' )->currencyConvert ( $regularpayment, $baseCurrencyCode, $currentCurrencyCode );
		$shipping = Mage::getSingleton ( 'checkout/session' )->getShipping ();
		if ($initialFeeEnabled && $trialEnabled) {
			return $trialAmount + $initialFee;
		} else if (! $trialEnabled) {
			if ($initialFeeEnabled) {
				return $initialFee + $regularpayment + $shipping;
			}
		} else {
			return $trialAmount + $shipping;
		}
		return $regularpayment + $shipping;
	}
			}
		}
		...
		if (Mage::getStoreConfigFlag ( 'recurringpayments/settings/active' ) ) {
			...
			if (! empty ( $productId ) && count ( $options ) >= 1) {
				if (isset ( $options [0] )) {
					...
					$trial = $trialAmount = Mage::getSingleton ( 'checkout/session' )->getTrialPrice ();
					$initial = $initialFee = Mage::getSingleton ( 'checkout/session' )->getInitialFee ();
					if ($row->getLabel () == "Initial Fee") {
						return $initialFee;
					} elseif ($row->getLabel () == "Trial Payment") {
						return $trialAmount;
					}
					if ($row->getLabel () == "Regular Payment") {
						Mage::getSingleton ( 'checkout/session' )->setRegularPayment ( $row->getAmount () );
					}
					if ($row->getLabel () == "Shipping") {
						Mage::getSingleton ( 'checkout/session' )->setShipping ( $row->getAmount () );
					}
