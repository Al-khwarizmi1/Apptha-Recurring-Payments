...
class Mage_Payment_Model_Recurring_Profile extends Mage_Core_Model_Abstract {
	...
	public function getValidationErrors($isGrouped = true, $asMessage = false) {
		if ($isGrouped && $this->_errors) {
			...
			if ($asMessage) {
				return Mage::throwException ( Mage::helper ( 'payment' )->__ ( "Payment profile is invalid:\n%s", implode ( "\n", $result ) ) );
			}
			return $result;
		}
		return $this->_errors;
	...
	public function importBuyRequest(Varien_Object $buyRequest) {
		...
		if ($startDate) {
			...
			$utcTime = $this->_locale->utcDate ( $this->_store, $startDate, true, $dateFormat )->toString ( Varien_Date::DATETIME_INTERNAL_FORMAT );
	...
	public function importProduct(Mage_Catalog_Model_Product $product) {
		if ($product->isRecurring () && is_array ( $product->getRecurringProfile () )) {
			...
			if (Mage::getStoreConfigFlag ( 'recurringpayments/settings/active' ) ) {
				...
				Mage::getSingleton ( 'checkout/session' )->setTrialPrice ( '' );
				Mage::getSingleton ( 'checkout/session' )->setTrialEnabled ( '' );
				Mage::getSingleton ( 'checkout/session' )->setInitialFeeEnabled ( '' );
				Mage::getSingleton ( 'checkout/session' )->setInitialFee ( '' );
				...
				if (! empty ( $productId ) && count ( $options ) >= 1) {
					if (isset ( $options [0] )) {
						...
						$productSubscription->getSelect ()->join ( $tablePrefix . 'apptha_subscriptiontypes', 'main_table.subscription_type =' . $tablePrefix . 'apptha_subscriptiontypes.id AND main_table.id = ' . $productSubscriptionId, array (
								...
								$tablePrefix . 'apptha_subscriptiontypes.is_infinite as is_infinite',
								$tablePrefix . 'apptha_subscriptiontypes.billing_period_unit as billing_period_unit',
								$tablePrefix . 'apptha_subscriptiontypes.occurrences_for_trialperiod as occurrences_for_trialperiod',
								$tablePrefix . 'apptha_subscriptiontypes.is_trial_enabled as is_trial_enabled',
								$tablePrefix . 'apptha_subscriptiontypes.is_initial_fee_enabled as is_initial_fee_enabled' 
						) );
						
						foreach ( $productSubscription as $subscription ) {
							
							$recurringProfile ['start_date_is_editable'] = 0;
							$recurringProfile ['schedule_description'] = $subscription->getTitle ();
							
							if ($subscription->getIsInfinite ()) {
								$recurringProfile ['bill_failed_later'] = 1;
								$recurringProfile ['period_frequency'] = $subscription->getBillingFrequency ();
								$recurringProfile ['period_max_cycles'] = '';
							} else {
								$recurringProfile ['period_frequency'] = $subscription->getBillingFrequency ();
								$recurringProfile ['period_max_cycles'] = $subscription->getBillingCycle ();
							}
							
							if ($subscription->getBillingPeriodUnit () == 4) {
								$recurringProfile ['period_unit'] = 'month';
							} elseif ($subscription->getBillingPeriodUnit () == 2) {
								$recurringProfile ['period_unit'] = 'week';
							} elseif ($subscription->getBillingPeriodUnit () == 5) {
								$recurringProfile ['period_unit'] = 'year';
							} else {
								$recurringProfile ['period_unit'] = 'day';
							}
							
							if ($subscription->getIsTrialEnabled ()) {
								
								Mage::getSingleton ( 'checkout/session' )->setTrialPrice ( $subscription->getTrialPeriodPrice () );
								Mage::getSingleton ( 'checkout/session' )->setTrialEnabled ( $subscription->getIsTrialEnabled () );
								$recurringProfile['trial_period_max_cycles']=$subscription->getOccurrencesForTrialperiod();
								$recurringProfile ['trial_billing_amount'] = $subscription->getTrialPeriodPrice ();
							    $recurringProfile['trial_period_unit']=$recurringProfile['period_unit'];
							    $recurringProfile['trial_period_frequency']=$recurringProfile['period_frequency'];
							} else {
								$recurringProfile ['trial_billing_amount'] = '';
							}
							
							if ($subscription->getIsInitialFeeEnabled ()) {
								Mage::getSingleton ( 'checkout/session' )->setInitialFeeEnabled ( $subscription->getIsInitialFeeEnabled () );
								$recurringProfile ['init_amount'] = $subscription->getInitialFee ();
								Mage::getSingleton ( 'checkout/session' )->setInitialFee ( $recurringProfile ['init_amount'] );
							} else {
								$recurringProfile ['init_amount'] = '';
							}
